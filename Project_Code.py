...
def home_page():
       ...
       ott_genre=px.bar(home_csv[selvalue].sort_values(),width=1250,color=home_csv['Genre that you prefer.'])
...
def netflix():
       ...
       if option1=='Genre Based Recommendation':
              ...
              unique_movie=df11['title'].unique()
              movie_namee=cols1.selectbox('Select Your Genre', unique_movie)
              def matrix_cosine():
                     ...
                     def select_movies(a=movie_namee):
                            ...
                            sorted_score = sorted_similar_movies[1:]
...
def primevideo():
       ...
       # charts 1
       d1 = final_dataset1.groupby(['type', 'year']).size().reset_index(name='Total Count')
       d1 = d1[d1['year'] >= 1928]
       fig3 = px.line(d1, x='year', y='Total Count', color='type', width=1500)
       fig3.update_layout(width=1100, hovermode='x unified')
       fig3.update_xaxes(showgrid=False)
       fig3.update_yaxes(showgrid=False)
       st.plotly_chart(fig3)
       ...
       # chart 2
       grouped = final_dataset1['type'].value_counts().reset_index()
       pie_chart = px.pie(values=grouped['type'], width=400,
                          hole=.3, hover_name=['Movies', 'TV Shows'])
       pie_chart.update_traces(marker=dict(line=dict(color='#000000', width=4)))
       col2.plotly_chart(pie_chart)

       # chart 3

       df_rating = final_dataset1[final_dataset1['type'] == type]
       totalrating = df_rating.groupby(by='genre')['genre'].count().sort_values(ascending=False)
       fig2 = px.bar(final_dataset1, y='rating', color_discrete_sequence=px.colors.sequential.OrRd, orientation='h')
       fig2.update_layout(xaxis_title='rating', yaxis_title='Total', height=400, width=700)
       col1.plotly_chart(fig2)
       st.sidebar.write(' ')
       sidebarslider = st.sidebar.slider('Year Wise Top Movies', 2009, 2021)
       ...
       if option1 == 'Genre Based Recommendation':
              ...
              genre_dataframe = genre_dataframe[['title', 'country', 'imdb_score', 'duration']].sort_values(
                     by='imdb_score', ascending=False).head(10)
              st.dataframe(genre_dataframe)

       elif option1 == 'Movie Description Based Recommendation':
              unique_movieprime = df1['title'].unique()
              movie_name = st.selectbox('Select Your Genre', unique_movieprime)
...
def hotstar():
       ...
       hotstar['type'] = hotstar['Type'].replace(['movie', 'tv'], ['Movie', 'Tv Show'])

       d1 = hotstar.groupby(['Type', 'Year']).size().reset_index(name='Total Count')
       d1 = d1[d1['Year'] >= 1928]
       fig3 = px.line(d1, x='Year', y='Total Count', color='Type',width=1200)
       fig3.update_layout(width=1100,hovermode='x unified')
       fig3.update_xaxes(showgrid=False)
       fig3.update_yaxes(showgrid=False)
       st.plotly_chart(fig3)
       ...
       grouped = d1['Type'].value_counts().reset_index()
       pie_chart = px.pie(values=grouped['Type'], width=400,
        	hole=.3, hover_name=['Movies','TV Shows'])
       pie_chart.update_traces(marker=dict(line=dict(color='#000000', width=4)))
       col2.plotly_chart(pie_chart)

       content_type_hotstar = st.sidebar.radio('Content Type', ['Movies', 'Tv Shows'])
       if content_type_hotstar == 'Movies':
              content_type_hotstar = 'movie'
       elif content_type_hotstar == 'Tv Shows':
              content_type_hotstar = 'tv'


       df_rating=hotstar[hotstar['Type']==content_type_hotstar]
       totalrating=df_rating.groupby(by='Imdb_Rating')['Imdb_Rating'].count().sort_values(ascending=False)
       fig2=px.bar(hotstar,y='Genre',color_discrete_sequence=px.colors.sequential.OrRd)
       fig2.update_layout(xaxis_title='IMDB Rating',yaxis_title='Total',height=400,width=700)
       col1.plotly_chart(fig2)

       type_movie = hotstar[hotstar['Type'] == 'Movie']
       type_tv = hotstar[hotstar['Type'] == 'Tv Show']

       type_tv.drop(['RunTime'], axis=1, inplace=True)
       type_tv = type_tv[['Title', 'Genre', 'Year', 'Imdb_Rating']]
       type_movie = type_movie[['Title', 'Genre', 'Year', 'Imdb_Rating', 'RunTime']]
       type_tv = type_tv.sort_values(by=['Imdb_Rating'], ascending=False)
       type_movie = type_movie.sort_values(by=['Imdb_Rating'], ascending=False)
       final_data = hotstar[['Hotstar_Id', 'Title', 'Description', 'Genre', 'Imdb_Rating']]
       ...
       if option1 == 'Genre Based Recommendation':
              option = columns1.selectbox('Select Your Genre', ['Comedy','Horror', 'Action','Drama'])
              df1=hotstar[hotstar['Genre']==option]
              st.dataframe(hotstar[['Title','Genre','Year']].head(10).reset_index())

       elif option1== 'Movie Description Based Recommendation':
              unique_moviehotstar = hotstar['Title'].unique()
              movie_namee = st.selectbox('Select Your Genre', unique_moviehotstar)
              def recommend(movie):
                     index = final_data[final_data['Title'] == movie].index[0]
                     distances = sorted(list(enumerate(similarity[index])), reverse=True, key=lambda x: x[1])
                     for i in distances[1:6]:
       ...
       st.sidebar.write(' ')
       sidebarslider11 = st.sidebar.slider('Year Wise Top Movies', 2009, 2021)

       hostar_sidebar = hotstar[hotstar['Year'] == sidebarslider11]
       hostar_sidebar = hostar_sidebar[hostar_sidebar['Type'] == content_type_hotstar]

       hostar_sidebar = hostar_sidebar.sort_values(by='Imdb_Rating', ascending=False)
       side_hotstar=hostar_sidebar[['Title','Imdb_Rating']].head(5)
       st.sidebar.dataframe(side_hotstar)
